...
export type Props = {
  ...
  nextHeaderLayoutY: number | undefined | null;
  ...
  scrollViewHeight: number | undefined | null;
};
type State = {
  measured: boolean;
  layoutY: number;
...
class ScrollViewStickyHeader extends React.Component<Props, State> {
  ...
  render(): React.Node {
      ...
      if (inverted) {
        ...
        if (scrollViewHeight != null) {
          ...
          if (stickStartPoint > 0) {
            ...
            outputRange.push(1); // If the next sticky header has not loaded yet (probably windowing) or is the last
        ...
        outputRange.push(0); // If the next sticky header has not loaded yet (probably windowing) or is the last
        // we can just keep it sticked forever.
    ...
    const child = React.Children.only(this.props.children);
    return <AnimatedView collapsable={false} onLayout={this._onLayout} style={[child.props.style, styles.header, {
      transform: [{
        translateY
      }]
    }]}>
