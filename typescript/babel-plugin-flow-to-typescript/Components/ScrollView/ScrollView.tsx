...
export type ScrollResponderType = {
  ...
  setNativeProps: ScrollView["setNativeProps"];
...
type IOSProps = Readonly<{
   ...
   */
  disableScrollViewPanResponder?: boolean | null;
...
type VRProps = Readonly<{
   ...
   * @platform vr
   */
  scrollBarThumbImage?: (Readonly<{}> | number) | null;
}>;
type StickyHeaderComponentType = React.AbstractComponent<ScrollViewStickyHeaderProps, Readonly<{
  setNextHeaderY: (a: number) => void;
}>>;
export type Props = Readonly<{
   ...
   * ```
   */
  contentContainerStyle?: ViewStyleProp | null;
   ...
   */
  invertStickyHeaders?: boolean | null;
   ...
   */
  keyboardDismissMode?: ("none" // cross-platform
  | "on-drag" | "interactive") | null;
  // ios only
   ...
   */
  keyboardShouldPersistTaps?: ("always" | "never" | "handled" | true | false) | null;
   ...
   */
  stickyHeaderIndices?: ReadonlyArray<number> | null;
...
type State = {
  layoutHeight: number | undefined | null;
...
const standardVerticalContext: ContextType = Object.freeze({
  horizontal: false
});
/**
...
class ScrollView extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    Object.keys(ScrollResponder.Mixin).filter(key => typeof ScrollResponder.Mixin[key] !== 'function').forEach(key => {
      // $FlowFixMe - dynamically adding properties to a class
  ...
  UNSAFE_componentWillReceiveProps(nextProps: Props) {
    const currentContentInsetTop = this.props.contentInset ? this.props.contentInset.top : 0;
    const nextContentInsetTop = nextProps.contentInset ? nextProps.contentInset.top : 0;
  ...
  getInnerViewNode(): number | undefined | null {
    return ReactNative.findNodeHandle(this._innerViewRef);
  }

  getInnerViewRef(): React.ElementRef<HostComponent<unknown>> | undefined | null {
    return this._innerViewRef;
  }

  getNativeScrollRef(): React.ElementRef<HostComponent<unknown>> | undefined | null {
  ...
  scrollTo(options?: {
    ...
    y?: number;
    animated?: boolean;
  } | number, deprecatedX?: number, deprecatedAnimated?: boolean) {
    let x, y, animated;
  ...
  _setStickyHeaderRef(key: string, ref?: React.ElementRef<StickyHeaderComponentType> | null) {
    if (ref) {
  ...
  _scrollViewRef: React.ElementRef<HostComponent<unknown>> | undefined | null = null;
  _setScrollViewRef = (ref?: React.ElementRef<HostComponent<unknown>> | null) => {
    this._scrollViewRef = ref;
  };
  _innerViewRef: React.ElementRef<HostComponent<unknown>> | undefined | null = null;
  _setInnerViewRef = (ref?: React.ElementRef<HostComponent<unknown>> | null) => {
  ...
  render(): React.Node | React.Element<string> {
    ...
    const contentContainerStyle = [this.props.horizontal === true && styles.contentContainerHorizontal, this.props.contentContainerStyle];
    ...
    if (stickyHeaderIndices != null && stickyHeaderIndices.length > 0) {
      ...
      children = childArray.map((child, index) => {
        ...
        if (indexOfIndex > -1) {
          ...
          return <StickyHeaderComponent key={key} ref={ref => this._setStickyHeaderRef(key, ref)} nextHeaderLayoutY={this._headerLayoutYs.get(this._getKeyForIndex(nextIndex, childArray))} onLayout={event => this._onStickyHeaderLayout(index, event, key)} scrollAnimatedValue={this._scrollAnimatedValue} inverted={this.props.invertStickyHeaders} scrollViewHeight={this.state.layoutHeight}>
              {child}
    ...
    children = <Context.Provider value={this.props.horizontal === true ? standardHorizontalContext : standardVerticalContext}>
        {children}
      </Context.Provider>;
    ...
    <ScrollContentContainerViewClass {...contentSizeChangeProps} ref={this._setInnerViewRef} style={contentContainerStyle} removeClippedSubviews={// Subview clipping causes issues with sticky headers on Android and
    // would be hard to fix properly in a performant way.
    Platform.OS === 'android' && hasStickyHeaders ? false : this.props.removeClippedSubviews} collapsable={false}>
        {children}
    ...
    const baseStyle = this.props.horizontal === true ? styles.baseHorizontal : styles.baseVertical;
    ...
    if (refreshControl) {
      if (Platform.OS === 'ios') {
        ...
        }, <ScrollViewClass {...props} style={[baseStyle, inner]} ref={this._setScrollViewRef}>
            {contentContainer}
          </ScrollViewClass>);
      }
    }

    return <ScrollViewClass {...props} ref={this._setScrollViewRef}>
        {contentContainer}
      </ScrollViewClass>;
  }
