...
const LISTVIEW_CELL_HEIGHT = 15; // Global id for the intercepted XMLHttpRequest objects.
...
type Props = Readonly<{}>;
type State = {
  detailRowId: number | undefined | null;
...
class NetworkOverlay extends React.Component<Props, State> {
  _requestsListView: React.ElementRef<typeof FlatList> | undefined | null;
  _detailScrollView: React.ElementRef<typeof ScrollView> | undefined | null; // Metrics are used to decide when if the request list should be sticky, and
  ...
  _socketIdMap = {}; // Map of `xhr._index` -> `index in `this.state.requests`.
  ...
  _renderItemDetail(id) {
    ...
    const details = Object.keys(requestItem).map(key => {
      ...
      return <View style={styles.detailViewRow} key={key}>
    ...
    return <View>
        <TouchableHighlight style={styles.closeButton} onPress={this._closeButtonClicked}>
          <View>
            <Text style={styles.closeButtonText}>v</Text>
          </View>
        </TouchableHighlight>
        <ScrollView style={styles.detailScrollView} ref={scrollRef => this._detailScrollView = scrollRef}>
          {details}
        </ScrollView>
      </View>;
  }
  ...
  _captureRequestsListView = (listRef?: FlatList<NetworkRequestInfo> | null): void => {
  ...
  render(): React.Node {
    ...
    return <View style={styles.container}>
        ...
        <View style={styles.listViewTitle}>
          {requests.length > 0 && <View style={styles.tableRow}>
        ...
        <FlatList ref={this._captureRequestsListView} onScroll={this._requestsListViewOnScroll} style={styles.listView} data={requests} renderItem={this._renderItem} keyExtractor={keyExtractor} extraData={this.state} />
      </View>;
  }
