...
const renderers = findRenderers(); // Required for React DevTools to view/edit React Native styles in Flipper.
...
class Inspector extends React.Component<{
  isFabric: boolean;
  inspectedView: HostRef | undefined | null;
  onRequestRerenderApp: (callback: (instance?: HostRef | null) => void) => void;
}, {
  devtoolsAgent: object | undefined | null;
  hierarchy: any;
  panelPos: string;
  inspecting: boolean;
  selection: number | undefined | null;
  perfing: boolean;
  inspected: any;
  inspectedView: HostRef | undefined | null;
  networking: boolean;
  ...
  _subs: Array<() => void> | undefined | null;
  ...
  componentDidMount() {
    hook.on('react-devtools', this._attachToDevtools); // if devtools is already started
  ...
  setSelection(i: number) {
    const hierarchyItem = this.state.hierarchy[i]; // we pass in ReactNative.findNodeHandle as the method is injected
