...
let _canInstallReactHook = false; // Implements a subset of User Timing API necessary for React measurements.
...
const userTimingPolyfill = __DEV__ ? {
  ...
  measure(measureName: string, startMark: string | undefined | null, endMark: string | undefined | null) {
    if (_enabled) {
  ...
  clearMeasures() {// React calls this to avoid memory leaks in browsers, but we don't keep
    // measurements anyway.
  }
