...
class PureComponentDebug<P extends DoNotCommitUsageOfPureComponentDebug, S extends object | undefined | null = void> extends React.Component<P, S> {
  shouldComponentUpdate(nextProps: P, nextState: S): boolean {
    ...
    if (prevPropsKeys.length !== nextPropsKeys.length) {
      ret = true;
      console.warn('PureComponentDebug: different prop keys', tag, prevPropsKeys.filter(key => !nextPropsKeys.includes(key)), nextPropsKeys.filter(key => !prevPropsKeys.includes(key)));
    }
    ...
    if (prevStateKeys.length !== nextStateKeys.length) {
      ret = true;
      console.warn('PureComponentDebug: different state keys', tag, prevStateKeys.filter(key => !nextStateKeys.includes(key)), nextStateKeys.filter(key => !prevStateKeys.includes(key)));
    }
