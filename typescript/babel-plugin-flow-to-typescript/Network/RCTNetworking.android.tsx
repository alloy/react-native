 ...
 * @flow
 */
'use strict'; // Do not require the native RCTNetworking module directly! Use this wrapper module instead.
...
type Header = [string, string]; // Convert FormData headers to arrays, which are easier to consume in
// native on Android.
...
class RCTNetworking extends NativeEventEmitter {
  ...
  sendRequest(method: string, trackingName: string, url: string, headers: object, data: RequestBody, responseType: "text" | "base64", incrementalUpdates: boolean, timeout: number, callback: (requestId: number) => unknown, withCredentials: boolean) {
    ...
    if (body && body.formData) {
      body.formData = body.formData.map(part => ({ ...part,
        headers: convertHeadersMapToArray(part.headers)
      }));
    }

    const requestId = generateRequestId();
    NativeNetworkingAndroid.sendRequest(method, url, requestId, convertHeadersMapToArray(headers), { ...body,
      trackingName
