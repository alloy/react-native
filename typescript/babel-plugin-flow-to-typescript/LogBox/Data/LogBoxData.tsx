...
export type Observer = (a: Readonly<{
  logs: LogBoxLogs;
  isDisabled: boolean;
...
let appInfo: (() => AppInfo) | undefined | null = null;
...
export function addLog(log: LogData): void {
  const errorForStackTrace = new Error(); // Parsing logs are expensive so we schedule this
...
export function getAppInfo(): AppInfo | undefined | null {
...
export function addIgnorePatterns(patterns: ReadonlyArray<IgnorePattern>): void {
  ...
  for (const pattern of newPatterns) {
    ignorePatterns.add(pattern); // We need to recheck all of the existing logs.
...
export function withSubscription(WrappedComponent: SubscribedComponent): React.AbstractComponent<{}> {
  class LogBoxStateSubscription extends React.Component<Props, State> {
    ...
    componentDidCatch(err: Error, errorInfo: {
      componentStack: string;
    }) {
      reportLogBoxError(err, errorInfo.componentStack);
    }

    _subscription: Subscription | undefined | null;
    state = {
      logs: new Set(),
      isDisabled: false,
      hasError: false,
    ...
    render(): React.Node {
      ...
      return <WrappedComponent logs={Array.from(this.state.logs)} isDisabled={this.state.isDisabled} selectedLogIndex={this.state.selectedLogIndex} />;
    }
