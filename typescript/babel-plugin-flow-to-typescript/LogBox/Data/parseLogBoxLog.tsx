...
export function parseLogBoxException(error: ExtendedExceptionData): LogBoxLogData {
  ...
  const level = message.match(/^TransformError /) ? 'syntax' : error.isFatal || error.isComponentError ? 'fatal' : 'error';
  return {
    level: level,
    stack: error.stack,
...
export function parseLogBoxLog(args: ReadonlyArray<unknown>): {
  ...
  let componentStack = []; // Extract component stack from warnings like "Some warning%s".

  if (typeof message === 'string' && message.slice(-2) === '%s' && args.length > 0) {
    const lastArg = args[args.length - 1]; // Does it look like React component stack? "   in ..."
  ...
  return { ...parseCategory(argsWithoutComponentStack),
    componentStack
  };
}