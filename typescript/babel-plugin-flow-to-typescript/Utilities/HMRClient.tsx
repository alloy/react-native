...
let pendingLogs: Array<[LogLevel, Array<unknown>]> = [];
type LogLevel = "trace" | "info" | "warn" | "log" | "group" | "groupCollapsed" | "groupEnd" | "debug";
...
const HMRClient: HMRClientNativeInterface = {
  enable() {
    ...
    const LoadingView = require('./LoadingView'); // We use this for internal logging only.
  ...
  log(level: LogLevel, data: Array<unknown>) {
    if (!hmrClient) {
    ...
    } catch (error) {// If sending logs causes any failures we want to silently ignore them
  ...
  setup(platform: string, bundleEntry: string, host: string, port: number | string, isEnabled: boolean) {
    invariant(platform, 'Missing required parameter `platform`');
    invariant(bundleEntry, 'Missing required parameter `bundleEntry`');
    invariant(host, 'Missing required parameter `host`');
    invariant(!hmrClient, 'Cannot initialize hmrClient twice'); // Moving to top gives errors due to NativeModules not being initialized
...
function setHMRUnavailableReason(reason) {
  ...
  hmrUnavailableReason = reason; // We only want to show a warning if Fast Refresh is on *and* if we ever
  // previously managed to connect successfully. We don't want to show
  // the warning to native engineers who use cached bundles without Metro.

  if (hmrClient.isEnabled() && didConnect) {
    console.warn(reason); // (Not using the `warning` module to prevent a Buck cycle.)
  }
}
...
function showCompileError() {
  ...
  currentCompileErrorMessage = null;
  const error: ExtendedError = new Error(message); // Symbolicating compile errors is wasted effort
