...
function maximumDepthError(tree: {
  toJSON: () => ReactTestRendererNode;
}, maxDepthLimit: number): string | undefined | null {
...
function expectNoConsoleWarn() {
  (jest as any).spyOn(console, 'warn').mockImplementation((...args) => {
...
function expectNoConsoleError() {
  ...
  (jest as any).spyOn(console, 'error').mockImplementation((...args) => {
    if (hasNotFailed) {
...
function expectRendersMatchingSnapshot(name: string, ComponentProvider: () => React.Element<any>, unmockComponent: () => unknown) {
  let instance;
  jest.resetAllMocks();
