...
export type JSTimerType = "setTimeout" | "setInterval" | "requestAnimationFrame" | "setImmediate" | "requestIdleCallback"; // These timing constants should be kept in sync with the ones in native ios and
...
const callbacks: Array<Function | undefined | null> = [];
const types: Array<JSTimerType | undefined | null> = [];
const timerIDs: Array<number | undefined | null> = [];
...
let GUID = 1;
let errors: Array<Error> | undefined | null = null;
let hasEmittedTimeDriftWarning = false; // Returns a free index if one is available, and the next consecutive index otherwise.
...
function _callTimer(timerID: number, frameTime: number, didTimeout?: boolean | null) {
  require('fbjs/lib/warning')(timerID <= GUID, 'Tried to call timer with ID %s but no such timer exists.', timerID); // timerIndex of -1 means that no timer with that ID exists. There are
...
function _freeCallback(timerID: number) {
  ...
  const index = timerIDs.indexOf(timerID); // See corresponding comment in `callTimers` for reasoning behind this
...
const JSTimers = {
  ...
  setTimeout: function (func: Function, duration: number, ...args: any): number {
    ...
    createTimer(id, duration || 0, Date.now(),
    /* recurring */
    false);
    return id;
  },
   ...
   */
  requestIdleCallback: function (func: Function, options: object | undefined | null) {
...
let ExportedJSTimers: {
  ...
  requestIdleCallback: (func: any, options?: any | null) => any | number;
