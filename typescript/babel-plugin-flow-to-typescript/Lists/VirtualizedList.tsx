...
type RequiredProps = {
   ...
   */
  getItem: (data: any, index: number) => Item | undefined | null;
...
type OptionalProps = {
   ...
   * your use-case.
   */
  ListItemComponent?: (React.ComponentType<any> | React.Element<any>) | null;

  /**
   * Rendered when the list is empty. Can be a React Component Class, a render function, or
   * a rendered element.
   */
  ListEmptyComponent?: (React.ComponentType<any> | React.Element<any>) | null;

  /**
   * Rendered at the bottom of all the items. Can be a React Component Class, a render function, or
   * a rendered element.
   */
  ListFooterComponent?: (React.ComponentType<any> | React.Element<any>) | null;
   ...
   * a rendered element.
   */
  ListHeaderComponent?: (React.ComponentType<any> | React.Element<any>) | null;
   ...
   */
  refreshControl?: React.Element<any> | null;
...
type ListDebugInfo = {
  cellKey: string;
  listKey: string;
  parent: ListDebugInfo | undefined | null;
...
class VirtualizedList extends React.PureComponent<Props, State> {
  props: Props; // scrollToEnd may be janky without getItemLayout prop
  ...
  getScrollResponder(): ScrollResponderType | undefined | null {
    if (this._scrollRef && this._scrollRef.getScrollResponder) {
      return this._scrollRef.getScrollResponder();
    }
  }

  getScrollableNode(): number | undefined | null {
  ...
  getScrollRef(): (React.ElementRef<typeof ScrollView> | undefined | null) | (React.ElementRef<typeof View> | undefined | null) {
  ...
  getChildContext(): {
    virtualizedList: {
      ...
      getNestedChildState: (a: string) => ChildListState | undefined | null;
      registerAsNestedChild: (a: {
        cellKey: string;
        key: string;
        ref: VirtualizedList;
        parentDebugInfo: ListDebugInfo;
      }) => ChildListState | undefined | null;
      unregisterAsNestedChild: (a: {
        key: string;
  ...
  _getNestedChildState = (key: string): ChildListState | undefined | null => {
  ...
  constructor(props: Props, context: object) {
    ...
    if (this._isNestedWithSameOrientation()) {
      const storedState = this.context.virtualizedList.getNestedChildState(this._getListKey());
  ...
  static getDerivedStateFromProps(newProps: Props, prevState: State): State {
    const {
      ...
      getItemCount,
      maxToRenderPerBatch
    } = newProps; // first and last could be stale (e.g. if a new, shorter items props is passed in), so we make
  ...
  _onUpdateSeparators = (keys: Array<string | undefined | null>, newProps: object) => {
    keys.forEach(key => {
  ...
  render(): React.Node {
    ...
    const inversionStyle = this.props.inverted ? this.props.horizontal ? styles.horizontallyInverted : styles.verticallyInverted : null;
    const cells = [];
    ...
    if (ListHeaderComponent) {
      ...
      const element = React.isValidElement(ListHeaderComponent) ? ListHeaderComponent : // $FlowFixMe
      <ListHeaderComponent />;
      cells.push(<VirtualizedCellWrapper cellKey={this._getCellKey() + '-header'} key="$header">
    ...
    if (itemCount > 0) {
      ...
      if (!isVirtualizationDisabled && last < itemCount - 1) {
        ...
        <View key="$tail_spacer" style={{
          [spacerKey]: tailSpacerLength
        }} />);
      }
    } else if (ListEmptyComponent) {
      const element: React.Element<any> = ((React.isValidElement(ListEmptyComponent) ? ListEmptyComponent : // $FlowFixMe
      <ListEmptyComponent />) as any);
      cells.push(React.cloneElement(element, {
        key: '$empty',
        onLayout: event => {
    ...
    if (ListFooterComponent) {
      const element = React.isValidElement(ListFooterComponent) ? ListFooterComponent : // $FlowFixMe
      <ListFooterComponent />;
      cells.push(<VirtualizedCellWrapper cellKey={this._getCellKey() + '-footer'} key="$footer">
    ...
    const innerRet = React.cloneElement((this.props.renderScrollComponent || this._defaultRenderScrollComponent)(scrollProps), {
      ref: this._captureScrollRef
    }, cells);
    let ret = innerRet;

    if (__DEV__) {
      ret = <ScrollView.Context.Consumer>
          {scrollContext => {
          if (scrollContext != null && !scrollContext.horizontal === !this.props.horizontal && !this._hasWarned.nesting && this.context.virtualizedList == null) {
    ...
    if (this.props.debug) {
      return <View style={styles.debug}>
          {ret}
  ...
  _averageCellLength = 0; // Maps a cell key to the set of keys for all outermost child lists within that cell
  ...
  _nestedChildLists: Map<string, {
    ref: VirtualizedList | undefined | null;
    state: ChildListState | undefined | null;
  }> = new Map();
  ...
  _scrollRef: React.ElementRef<any> | undefined | null = null;
  ...
  _defaultRenderScrollComponent = props => {
    ...
    if (this._isNestedWithSameOrientation()) {
      ...
      return (// $FlowFixMe Invalid prop usage
        <ScrollView {...props} refreshControl={props.refreshControl == null ? <RefreshControl refreshing={props.refreshing} onRefresh={onRefresh} progressViewOffset={props.progressViewOffset} /> : props.refreshControl} />
      );
    } else {
  ...
  measureLayoutRelativeToContainingList(): void {
    ...
    try {
      ...
      this._scrollRef.measureLayout(this.context.virtualizedList.getOutermostParentListRef().getScrollRef().getNativeScrollRef(), (x, y, width, height) => {
        ...
        const scrollMetrics = this._convertParentScrollMetrics(this.context.virtualizedList.getScrollMetrics());
  ...
  _renderDebugOverlay() {
    ...
    return <View style={[styles.debugOverlayBase, styles.debugOverlay]}>
        {framesInLayout.map((f, ii) => <View key={'f' + ii} style={[styles.debugOverlayBase, styles.debugOverlayFrame, {
        top: f.offset * normalize,
        height: f.length * normalize
      }]} />)}
        <View style={[styles.debugOverlayBase, styles.debugOverlayFrameLast, {
        top: windowTop * normalize,
        height: windowLen * normalize
      }]} />
        <View style={[styles.debugOverlayBase, styles.debugOverlayFrameVis, {
  ...
  _updateCellsToRender = () => {
    ...
    this.setState(state => {
      ...
      if (!isVirtualizationDisabled) {
         ...
         * delete this comment and run Flow. */
        distanceFromEnd < onEndReachedThreshold * visibleLength ? this.props.maxToRenderPerBatch : 0;
        newState = {
          first: 0,
...
type CellRendererProps = {
  CellRendererComponent?: React.ComponentType<any> | null;
  ItemSeparatorComponent: React.ComponentType<any> | undefined | null;
  cellKey: string;
  ...
  onUpdateSeparators: (cellKeys: Array<string | undefined | null>, props: object) => void;
  parentProps: {
    // e.g. height, y,
    getItemLayout?: (data: any, index: number) => {
      length: number;
  ...
  prevCellKey: string | undefined | null;
};
type CellRendererState = {
  separatorProps: Readonly<{
    highlighted: boolean;
    leadingItem: Item | undefined | null;
  }>;
};

class CellRenderer extends React.Component<CellRendererProps, CellRendererState> {
  state = {
    separatorProps: {
      highlighted: false,
  ...
  static getDerivedStateFromProps(props: CellRendererProps, prevState: CellRendererState): CellRendererState | undefined | null {
    return {
      separatorProps: { ...prevState.separatorProps,
  ...
  updateSeparatorProps(newProps: object) {
    this.setState(state => ({
      separatorProps: { ...state.separatorProps,
        ...newProps
      }
    }));
  }
  ...
  render() {
    ...
    const itemSeparator = ItemSeparatorComponent && <ItemSeparatorComponent {...this.state.separatorProps} />;
    ...
    return <CellRendererComponent {...this.props} style={cellStyle} onLayout={onLayout}>
        {element}
        {itemSeparator}
      </CellRendererComponent>;
  }
...
function describeNestedLists(childList: {
  ...
  readonly key: string;
  readonly ref: VirtualizedList;
  readonly parentDebugInfo: ListDebugInfo;
