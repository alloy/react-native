...
type RequiredProps<ItemT> = {
   ...
   */
  data: ReadonlyArray<ItemT> | undefined | null;
};
type OptionalProps<ItemT> = {
   ...
   */
  getItemLayout?: (data: Array<ItemT> | undefined | null, index: number) => {
    length: number;
    offset: number;
    index: number;
  };
...
class FlatList<ItemT> extends React.PureComponent<Props<ItemT>, void> {
  ...
  getNativeScrollRef(): (React.ElementRef<typeof View> | undefined | null) | (React.ElementRef<ScrollViewNativeComponentType> | undefined | null) {
    if (this._listRef) {
  ...
  _listRef: React.ElementRef<typeof VirtualizedList> | undefined | null;
  ...
  _getItemCount = (data?: Array<ItemT> | null): number => {
    if (data) {
      const {
        numColumns
      } = this.props;
  ...
  _renderer = () => {
    ...
    let virtualizedListRenderKey = ListItemComponent ? 'ListItemComponent' : 'renderItem';
    ...
    return {
      ...
      [virtualizedListRenderKey]: (info: RenderItemProps<ItemT>) => {
        if (numColumns > 1) {
          ...
          return <View style={StyleSheet.compose(styles.row, columnWrapperStyle)}>
              {item.map((it, kk) => {
  ...
  render(): React.Node {
    ...
    return <VirtualizedList {...restProps} getItem={this._getItem} getItemCount={this._getItemCount} keyExtractor={this._keyExtractor} ref={this._captureRef} viewabilityConfigCallbackPairs={this._virtualizedListPairs} {...this._renderer()} />;
  }
