...
type Props = Readonly<{}>;
type State = {
  registry: Registry | undefined | null;
};
let YellowBox;
/**
...
if (__DEV__) {
  ...
  const LogBoxNotificationContainer = require('../LogBox/LogBoxNotificationContainer').default; // YellowBox needs to insert itself early,
  ...
  YellowBox = class YellowBox extends React.Component<Props, State> {
    ...
    static __unstable_enableLogBox(): void {
      ...
      _isLogBoxEnabled = true; // TODO: Temporary hack to prevent cycles with the ExceptionManager.
  ...
  const registerError = (...args): void => {
    ...
    if (filterResult.forceDialogImmediately === true) {
      ...
      args[0] = `Warning: ${filterResult.finalFormat}`; // Note: YellowBox has no concept of "soft errors" so we're showing YellowBox for those.
...
module.exports = (YellowBox as Class<React.Component<Props, State>> & {
  ignoreWarnings(a: ReadonlyArray<IgnorePattern>): void;
  install(): void;
  uninstall(): void;
