...
class YellowBoxWarning {
  static parse({
    ...
    let mutableArgs: Array<unknown> = [...args]; // This detects a very narrow case of a simple warning string,
    ...
    return { ...YellowBoxCategory.parse(mutableArgs),
  ...
  symbolicate(callback: () => void): SymbolicationRequest {
    ...
    if (this.symbolicated.status !== 'COMPLETE') {
      const updateStatus = (error?: Error | null, stack?: Stack | null): void => {
        if (error != null) {
          this.symbolicated = {
            error,
            stack: null,
