yarn run v1.21.1
$ /Users/eloy/Code/ReactNative/react-native/node_modules/.bin/flow-to-ts Libraries/Renderer/implementations/ReactNativeRenderer-prod.fb.js
"use strict";
  ...
  switch (workInProgress.tag) {
    ...
    case 19:
      ...
      if (null === tag) {
        ...
        } else 2 * now() - newProps.renderingStartTime > newProps.tailExpiration && 1 < renderExpirationTime$jscomp$0 && (workInProgress.effectTag |= 64, rootContainerInstance = !0, cutOffTailIfNeeded(newProps, !1), workInProgress.expirationTime = workInProgress.childExpirationTime = renderExpirationTime$jscomp$0 - 1);
...
function ensureRootIsScheduled(root) {
  if (0 !== root.lastExpiredTime) root.callbackExpirationTime = 1073741823, root.callbackPriority = 99, root.callbackNode = scheduleSyncCallback(performSyncWorkOnRoot.bind(null, root));else {
    ...
    if (0 === expirationTime) null !== existingCallbackNode && (root.callbackNode = null, root.callbackExpirationTime = 0, root.callbackPriority = 90);else {
      ...
      expirationTime = 1073741823 === expirationTime ? scheduleSyncCallback(performSyncWorkOnRoot.bind(null, root)) : scheduleCallback(priorityLevel, performConcurrentWorkOnRoot.bind(null, root), { timeout: 10 * (1073741821 - expirationTime) - now() });
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (0 !== expirationTime) {
    ...
    flushPassiveEffects();
    root === workInProgressRoot && expirationTime === renderExpirationTime || prepareFreshStack(root, expirationTime);
...
function performSyncWorkOnRoot(root) {
  ...
  if ((executionContext & (RenderContext | CommitContext)) !== NoContext) throw Error("Should not already be working.");
  flushPassiveEffects();
  root === workInProgressRoot && lastExpiredTime === renderExpirationTime || prepareFreshStack(root, lastExpiredTime);
  if (null !== workInProgress) {
    var prevExecutionContext = executionContext;
    executionContext |= RenderContext;
    var prevDispatcher = pushDispatcher(root);
  ...
  do {
    ...
    if (0 === (workInProgress.effectTag & 2048)) {
      ...
      if (1 === renderExpirationTime || 1 !== completedWork.childExpirationTime) {
        for (var newChildExpirationTime = 0, _child = completedWork.child; null !== _child;) {
