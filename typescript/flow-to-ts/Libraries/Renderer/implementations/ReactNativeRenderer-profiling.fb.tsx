yarn run v1.21.1
$ /Users/eloy/Code/ReactNative/react-native/node_modules/.bin/flow-to-ts Libraries/Renderer/implementations/ReactNativeRenderer-profiling.fb.js
"use strict";
  ...
  if (eventPluginOrder) for (var pluginName in namesToPlugins) {
    ...
    if (!plugins[pluginIndex]) {
      ...
      for (var eventName in pluginIndex) {
        ...
        if (eventNameDispatchConfigs.hasOwnProperty(eventName$jscomp$0)) throw Error("EventPluginHub: More than one plugin attempted to publish the same event name, `" + eventName$jscomp$0 + "`.");
...
function publishRegistrationName(registrationName, pluginModule) {
  if (registrationNameModules[registrationName]) throw Error("EventPluginHub: More than one plugin attempted to publish the same registration name, `" + registrationName + "`.");
...
function getListener(inst, registrationName) {
  ...
  a: switch (registrationName) {
    case "onClick":case "onClickCapture":case "onDoubleClick":case "onDoubleClickCapture":case "onMouseDown":case "onMouseDownCapture":case "onMouseMove":case "onMouseMoveCapture":case "onMouseUp":case "onMouseUpCapture":
...
function restoreStateOfTarget(target) {
  if (getInstanceFromNode(target)) throw Error("setRestoreImplementation() needs to be called to handle a target for controlled events. This error is likely caused by a bug in React. Please file an issue.");
}
...
function batchedUpdates(fn, bookkeeping) {
  ...
  try {
    ...
    if (isInsideEventHandler = !1, null !== restoreTarget || null !== restoreQueue) if (flushDiscreteUpdatesImpl(), restoreTarget && (bookkeeping = restoreTarget, fn = restoreQueue, restoreQueue = restoreTarget = null, restoreStateOfTarget(bookkeeping), fn)) for (bookkeeping = 0; bookkeeping < fn.length; bookkeeping++) restoreStateOfTarget(fn[bookkeeping]);
  }
}
...
function _receiveRootNodeIDEvent(rootNodeID, topLevelType, nativeEventParam) {
  var nativeEvent = nativeEventParam || EMPTY_NATIVE_EVENT,
      ...
      target = null;
  enableNativeTargetAsInstance ? null != inst && (target = inst.stateNode) : target = nativeEvent.target;
...
function restoreDeletedValuesInNestedArray(updatePayload, node, validAttributes) {
  if (Array.isArray(node)) for (var i = node.length; i-- && 0 < removedKeyCount;) restoreDeletedValuesInNestedArray(updatePayload, node[i], validAttributes);else if (node && 0 < removedKeyCount) for (i in removedKeys) if (removedKeys[i]) {
    var nextProp = node[i];
    if (void 0 !== nextProp) {
...
function getStackByFiberInDevAndProd(workInProgress) {
  ...
  do {
    a: switch (workInProgress.tag) {
      case 3:case 4:case 6:case 7:case 10:case 9:
...
function processChildContext(fiber, type, parentContext) {
  ...
  for (var contextKey in instance) if (!(contextKey in fiber)) throw Error((getComponentName(type) || "Unknown") + '.getChildContext(): key "' + contextKey + '" is not defined in childContextTypes.');
...
function shallowEqual(objA, objB) {
  ...
  for (keysB = 0; keysB < keysA.length; keysB++) if (!hasOwnProperty.call(objB, keysA[keysB]) || !is$1(objA[keysA[keysB]], objB[keysA[keysB]])) return !1;
  return !0;
}
...
function coerceRef(returnFiber, current$$1, element) {
  ...
  if (null !== returnFiber && "function" !== typeof returnFiber && "object" !== typeof returnFiber) {
    if (element._owner) {
      ...
      if (element) {
        if (1 !== element.tag) throw Error("Function components cannot have refs. Did you mean to use React.forwardRef()?");
              ...
              switch (isUnkeyedTopLevelFragment.tag) {
                case 7:
                  if (newChild.type === REACT_FRAGMENT_TYPE) {
                    ...
                    currentFirstChild = useFiber(isUnkeyedTopLevelFragment, newChild.props.children, expirationTime);
                ...
                default:
                  if (isUnkeyedTopLevelFragment.elementType === newChild.type) {
                    deleteRemainingChildren(returnFiber, isUnkeyedTopLevelFragment.sibling);
                    currentFirstChild = useFiber(isUnkeyedTopLevelFragment, newChild.props, expirationTime);
...
function pushHostContext(fiber) {
  ...
  nextContext = context.isInAParentText !== nextContext ? { isInAParentText: nextContext } : context;
...
function findFirstSuspended(row) {
  for (var node = row; null !== node;) {
    if (13 === node.tag) {
      ...
      if (null !== state && (state = state.dehydrated, null === state || shim$1(state) || shim$1(state))) return node;
...
function areHookInputsEqual(nextDeps, prevDeps) {
  ...
  for (var i = 0; i < prevDeps.length && i < nextDeps.length; i++) if (!is$1(nextDeps[i], prevDeps[i])) return !1;
  return !0;
}
function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderExpirationTime) {
  ...
  if (didScheduleRenderPhaseUpdate) {
    do didScheduleRenderPhaseUpdate = !1, numberOfReRenders += 1, workInProgressHook = currentHook = null, workInProgress.updateQueue = null, ReactCurrentDispatcher$1.current = HooksDispatcherOnUpdate, current = Component(props, secondArg); while (didScheduleRenderPhaseUpdate);
    renderPhaseUpdates = null;
    numberOfReRenders = 0;
  }
  ...
  if (0 < numberOfReRenders) {
    ...
    if (null !== renderPhaseUpdates) {
      ...
      if (void 0 !== firstRenderPhaseUpdate) {
        ...
        do newState = reducer(newState, firstRenderPhaseUpdate.action), firstRenderPhaseUpdate = firstRenderPhaseUpdate.next; while (null !== firstRenderPhaseUpdate);
  ...
  if (null !== _dispatch) {
    ...
    do {
      ...
      if (updateExpirationTime < renderExpirationTime$1) {
        ...
        null === newBaseQueueLast ? (baseFirst = newBaseQueueLast = clone, firstRenderPhaseUpdate = newState) : newBaseQueueLast = newBaseQueueLast.next = clone;
        ...
        eagerState: _update.eagerState,
        next: null
      }), markRenderEventTimeAndConfig(updateExpirationTime, _update.suspenseConfig), newState = _update.eagerReducer === reducer ? _update.eagerState : reducer(newState, _update.action);
      _update = _update.next;
    } while (null !== _update && _update !== _dispatch);
    null === newBaseQueueLast ? firstRenderPhaseUpdate = newState : newBaseQueueLast.next = baseFirst;
    is$1(newState, hook.memoizedState) || (didReceiveUpdate = !0);
    hook.memoizedState = newState;
...
function tryToClaimNextHydratableInstance(fiber$jscomp$0) {
  if (isHydrating) {
    ...
    } else fiber$jscomp$0.effectTag = fiber$jscomp$0.effectTag & -1025 | 2, isHydrating = !1, hydrationParentFiber = fiber$jscomp$0;
  }
}
...
function updateSuspenseComponent(current$$1, workInProgress, renderExpirationTime) {
  var mode = workInProgress.mode,
      ...
      nextDidTimeout = !1,
      JSCompiler_temp;
  (JSCompiler_temp = 0 !== (workInProgress.effectTag & 64)) || (JSCompiler_temp = 0 !== (suspenseContext & 2) && (null === current$$1 || null !== current$$1.memoizedState));
  JSCompiler_temp ? (nextDidTimeout = !0, workInProgress.effectTag &= -65) : null !== current$$1 && null === current$$1.memoizedState || void 0 === nextProps.fallback || !0 === nextProps.unstable_avoidThisFallback || (suspenseContext |= 1);
  ...
  if (null === current$$1) {
    ...
    if (nextDidTimeout) {
      ...
      if (0 === (workInProgress.mode & 2)) for (current$$1 = null !== workInProgress.memoizedState ? workInProgress.child.child : workInProgress.child, nextProps.child = current$$1; null !== current$$1;) current$$1.return = nextProps, current$$1 = current$$1.sibling;
  ...
  if (null !== current$$1.memoizedState) {
    ...
    if (nextDidTimeout) {
      ...
      renderExpirationTime = createWorkInProgress(current$$1, current$$1.pendingProps, 0);
      renderExpirationTime.return = workInProgress;
      if (0 === (workInProgress.mode & 2) && (nextDidTimeout = null !== workInProgress.memoizedState ? workInProgress.child.child : workInProgress.child, nextDidTimeout !== current$$1.child)) for (renderExpirationTime.child = nextDidTimeout; null !== nextDidTimeout;) nextDidTimeout.return = renderExpirationTime, nextDidTimeout = nextDidTimeout.sibling;
      if (workInProgress.mode & 8) {
        nextDidTimeout = 0;
        for (current$$1 = renderExpirationTime.child; null !== current$$1;) nextDidTimeout += current$$1.treeBaseDuration, current$$1 = current$$1.sibling;
    ...
    renderExpirationTime = reconcileChildFibers(workInProgress, current$$1.child, nextProps.children, renderExpirationTime);
    workInProgress.memoizedState = null;
  ...
  if (nextDidTimeout) {
    ...
    if (workInProgress.mode & 8) {
      current$$1 = 0;
      for (suspenseContext = nextProps.child; null !== suspenseContext;) current$$1 += suspenseContext.treeBaseDuration, suspenseContext = suspenseContext.sibling;
  ...
  workInProgress.memoizedState = null;
  return workInProgress.child = reconcileChildFibers(workInProgress, current$$1, nextProps.children, renderExpirationTime);
}
...
function updateSuspenseListComponent(current$$1, workInProgress, renderExpirationTime) {
  ...
  if (0 === (workInProgress.mode & 2)) workInProgress.memoizedState = null;else switch (revealOrder) {
    case "forwards":
      ...
      for (revealOrder = null; null !== renderExpirationTime;) current$$1 = renderExpirationTime.alternate, null !== current$$1 && null === findFirstSuspended(current$$1) && (revealOrder = renderExpirationTime), renderExpirationTime = renderExpirationTime.sibling;
...
function bailoutOnAlreadyFinishedWork(current$$1, workInProgress, renderExpirationTime) {
  ...
  if (null !== current$$1 && workInProgress.child !== current$$1.child) throw Error("Resuming work not yet implemented.");
  if (null !== workInProgress.child) {
    ...
    workInProgress.child = renderExpirationTime;
    for (renderExpirationTime.return = workInProgress; null !== current$$1.sibling;) current$$1 = current$$1.sibling, renderExpirationTime = renderExpirationTime.sibling = createWorkInProgress(current$$1, current$$1.pendingProps, current$$1.expirationTime), renderExpirationTime.return = workInProgress;
...
function completeWork(current, workInProgress, renderExpirationTime$jscomp$0) {
  ...
  switch (workInProgress.tag) {
    case 2:case 16:case 15:case 0:case 11:case 7:case 8:case 12:case 9:case 14:
      return null;
    case 1:
      return isContextProvider(workInProgress.type) && popContext(workInProgress), null;
    case 3:
      return popHostContainer(workInProgress), popTopLevelContextObject(workInProgress), current = workInProgress.stateNode, current.pendingContext && (current.context = current.pendingContext, current.pendingContext = null), updateHostContainer(workInProgress), null;
    case 5:
      ...
      if (null !== current && null != workInProgress.stateNode) updateHostComponent$1(current, workInProgress, renderExpirationTime$jscomp$0, newProps, rootContainerInstance), current.ref !== workInProgress.ref && (workInProgress.effectTag |= 128);else {
        if (!newProps) {
        ...
        var tag = allocateTag(),
            ...
            updatePayload = diffProperties(null, emptyObject, newProps, viewConfig.validAttributes);
        ...
        finalizeInitialChildren(viewConfig, renderExpirationTime$jscomp$0, newProps, rootContainerInstance, current) && (workInProgress.effectTag |= 4);
    ...
    case 6:
      if (current && null != workInProgress.stateNode) updateHostText$1(current, workInProgress, current.memoizedProps, newProps);else {
        ...
        ReactNativePrivateInterface.UIManager.createView(rootContainerInstance, "RCTRawText", current, { text: newProps });
    ...
    case 13:
      ...
      if (0 !== (workInProgress.effectTag & 64)) return workInProgress.expirationTime = renderExpirationTime$jscomp$0, workInProgress;
      newProps = null !== newProps;
      rootContainerInstance = !1;
      null !== current && (renderExpirationTime$jscomp$0 = current.memoizedState, rootContainerInstance = null !== renderExpirationTime$jscomp$0, newProps || null === renderExpirationTime$jscomp$0 || (renderExpirationTime$jscomp$0 = current.child.sibling, null !== renderExpirationTime$jscomp$0 && (tag = workInProgress.firstEffect, null !== tag ? (workInProgress.firstEffect = renderExpirationTime$jscomp$0, renderExpirationTime$jscomp$0.nextEffect = tag) : (workInProgress.firstEffect = workInProgress.lastEffect = renderExpirationTime$jscomp$0, renderExpirationTime$jscomp$0.nextEffect = null), renderExpirationTime$jscomp$0.effectTag = 8)));
      if (newProps && !rootContainerInstance && 0 !== (workInProgress.mode & 2)) if (null === current && !0 !== workInProgress.memoizedProps.unstable_avoidThisFallback || 0 !== (suspenseStackCursor.current & 1)) workInProgressRootExitStatus === RootIncomplete && (workInProgressRootExitStatus = RootSuspended);else {
        ...
        0 !== workInProgressRootNextUnprocessedUpdateTime && null !== workInProgressRoot && (markRootSuspendedAtTime(workInProgressRoot, renderExpirationTime), markRootUpdatedAtTime(workInProgressRoot, workInProgressRootNextUnprocessedUpdateTime));
      }
    ...
    case 19:
      ...
      if (null === tag) {
        if (rootContainerInstance) cutOffTailIfNeeded(newProps, !1);else {
          if (workInProgressRootExitStatus !== RootIncomplete || null !== current && 0 !== (current.effectTag & 64)) for (current = workInProgress.child; null !== current;) {
            ...
            if (null !== tag) {
              ...
              for (newProps = workInProgress.child; null !== newProps;) rootContainerInstance = newProps, tag = current, rootContainerInstance.effectTag &= 2, rootContainerInstance.nextEffect = null, rootContainerInstance.firstEffect = null, rootContainerInstance.lastEffect = null, renderExpirationTime$jscomp$0 = rootContainerInstance.alternate, null === renderExpirationTime$jscomp$0 ? (rootContainerInstance.childExpirationTime = 0, rootContainerInstance.expirationTime = tag, rootContainerInstance.child = null, rootContainerInstance.memoizedProps = null, rootContainerInstance.memoizedState = null, rootContainerInstance.updateQueue = null, rootContainerInstance.dependencies = null, rootContainerInstance.selfBaseDuration = 0, rootContainerInstance.treeBaseDuration = 0) : (rootContainerInstance.childExpirationTime = renderExpirationTime$jscomp$0.childExpirationTime, rootContainerInstance.expirationTime = renderExpirationTime$jscomp$0.expirationTime, rootContainerInstance.child = renderExpirationTime$jscomp$0.child, rootContainerInstance.memoizedProps = renderExpirationTime$jscomp$0.memoizedProps, rootContainerInstance.memoizedState = renderExpirationTime$jscomp$0.memoizedState, rootContainerInstance.updateQueue = renderExpirationTime$jscomp$0.updateQueue, tag = renderExpirationTime$jscomp$0.dependencies, rootContainerInstance.dependencies = null === tag ? null : {
                expirationTime: tag.expirationTime,
                firstContext: tag.firstContext,
                responders: tag.responders
              }, rootContainerInstance.selfBaseDuration = renderExpirationTime$jscomp$0.selfBaseDuration, rootContainerInstance.treeBaseDuration = renderExpirationTime$jscomp$0.treeBaseDuration), newProps = newProps.sibling;
        ...
        if (!rootContainerInstance) if (current = findFirstSuspended(tag), null !== current) {
          if (workInProgress.effectTag |= 64, rootContainerInstance = !0, current = current.updateQueue, null !== current && (workInProgress.updateQueue = current, workInProgress.effectTag |= 4), cutOffTailIfNeeded(newProps, !0), null === newProps.tail && "hidden" === newProps.tailMode && !tag.alternate) return workInProgress = workInProgress.lastEffect = newProps.lastEffect, null !== workInProgress && (workInProgress.nextEffect = null), null;
        } else 2 * now() - newProps.renderingStartTime > newProps.tailExpiration && 1 < renderExpirationTime$jscomp$0 && (workInProgress.effectTag |= 64, rootContainerInstance = !0, cutOffTailIfNeeded(newProps, !1), current = renderExpirationTime$jscomp$0 - 1, workInProgress.expirationTime = workInProgress.childExpirationTime = current, null === spawnedWorkDuringRender ? spawnedWorkDuringRender = [current] : spawnedWorkDuringRender.push(current));
        newProps.isBackwards ? (tag.sibling = workInProgress.child, workInProgress.child = tag) : (current = newProps.last, null !== current ? current.sibling = tag : workInProgress.child = tag, newProps.last = tag);
      }
...
function unwindWork(workInProgress) {
  switch (workInProgress.tag) {
    ...
    case 13:
      return pop(suspenseStackCursor, workInProgress), effectTag = workInProgress.effectTag, effectTag & 4096 ? (workInProgress.effectTag = effectTag & -4097 | 64, workInProgress) : null;
    case 19:
...
function commitBeforeMutationLifeCycles(current$$1, finishedWork) {
  switch (finishedWork.tag) {
    case 0:case 11:case 15:case 22:
    ...
    case 1:
      ...
      return;
    case 3:case 5:case 6:case 4:case 17:
      return;
...
function commitHookEffectList(unmountTag, mountTag, finishedWork) {
  ...
  if (null !== finishedWork) {
    ...
    do {
      ...
      0 !== (effect.tag & mountTag) && (destroy = effect.create, effect.destroy = destroy());
      effect = effect.next;
...
function commitLifeCycles(finishedRoot, current$$1, finishedWork, committedExpirationTime) {
  switch (finishedWork.tag) {
    ...
    case 1:
      ...
      if (finishedWork.effectTag & 4) if (null === current$$1) finishedRoot.componentDidMount();else {
        var prevProps = finishedWork.elementType === finishedWork.type ? current$$1.memoizedProps : resolveDefaultProps(finishedWork.type, current$$1.memoizedProps);
        finishedRoot.componentDidUpdate(prevProps, current$$1.memoizedState, finishedRoot.__reactInternalSnapshotBeforeUpdate);
      }
      current$$1 = finishedWork.updateQueue;
      null !== current$$1 && commitUpdateQueue(finishedWork, current$$1, finishedRoot, committedExpirationTime);
      return;
    case 3:
      current$$1 = finishedWork.updateQueue;
      ...
      if (null !== current$$1) {
        ...
        commitUpdateQueue(finishedWork, current$$1, finishedRoot, committedExpirationTime);
      }
    ...
    case 12:
      ...
      "function" === typeof committedExpirationTime && committedExpirationTime(finishedWork.memoizedProps.id, null === current$$1 ? "mount" : "update", finishedWork.actualDuration, finishedWork.treeBaseDuration, finishedWork.actualStartTime, commitTime, finishedRoot.memoizedInteractions);
      return;
    case 13:
      return;
...
function commitUnmount(finishedRoot, current$$1$jscomp$0, renderPriorityLevel) {
  ...
  switch (current$$1$jscomp$0.tag) {
    case 0:case 11:case 14:case 15:case 22:
    ...
    case 5:
      ...
      break;
    case 4:
      unmountHostComponents(finishedRoot, current$$1$jscomp$0, renderPriorityLevel);
...
function unmountHostComponents(finishedRoot$jscomp$0, current$$1, renderPriorityLevel$jscomp$0) {
  for (var node = current$$1, currentParentIsValid = !1, currentParent, currentParentIsContainer;;) {
...
function commitWork(current$$1, finishedWork) {
  switch (finishedWork.tag) {
    ...
    case 6:
      ...
      ReactNativePrivateInterface.UIManager.updateView(finishedWork.stateNode, "RCTRawText", { text: finishedWork.memoizedProps });
      return;
    case 3:
      return;
    case 12:
      ...
      if (null !== instance) a: for (current$$1 = instance;;) {
        if (5 === current$$1.tag) {
          if (updatePayload = current$$1.stateNode, newProps) {
            ...
            var updatePayload$jscomp$0 = diffProperties(null, emptyObject, { style: { display: "none" } }, viewConfig.validAttributes);
          ...
          if (13 === current$$1.tag && null !== current$$1.memoizedState && null === current$$1.memoizedState.dehydrated) {
...
function markUpdateTimeFromFiberToRoot(fiber, expirationTime) {
  ...
  null !== root && (workInProgressRoot === root && (markUnprocessedUpdateTime(expirationTime), workInProgressRootExitStatus === RootSuspendedWithDelay && markRootSuspendedAtTime(root, renderExpirationTime)), markRootUpdatedAtTime(root, expirationTime));
  return root;
}
  ...
  if (0 !== root.lastExpiredTime) root.callbackExpirationTime = 1073741823, root.callbackPriority = 99, root.callbackNode = scheduleSyncCallback(performSyncWorkOnRoot.bind(null, root));else {
    ...
    if (0 === expirationTime) null !== existingCallbackNode && (root.callbackNode = null, root.callbackExpirationTime = 0, root.callbackPriority = 90);else {
      ...
      if (null !== existingCallbackNode) {
        ...
        if (root.callbackExpirationTime === expirationTime && existingCallbackPriority >= currentTime) return;
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (0 !== expirationTime) {
    ...
    flushPassiveEffects();
    if (root !== workInProgressRoot || expirationTime !== renderExpirationTime) prepareFreshStack(root, expirationTime), startWorkOnPendingInteractions(root, expirationTime);
      ...
      if (null === workInProgress) switch (prevDispatcher = root.finishedWork = root.current.alternate, root.finishedExpirationTime = expirationTime, prevExecutionContext = workInProgressRootExitStatus, workInProgressRoot = null, prevExecutionContext) {
        ...
        case RootSuspended:
          ...
          if (1073741823 === workInProgressRootLatestProcessedExpirationTime && (prevDispatcher = globalMostRecentFallbackTime + FALLBACK_THROTTLE_MS - now(), 10 < prevDispatcher)) {
            ...
            if (0 !== prevInteractions && prevInteractions !== expirationTime) break;
        ...
        case RootCompleted:
          if (1073741823 !== workInProgressRootLatestProcessedExpirationTime && null !== workInProgressRootCanSuspendUsingConfig) {
            ...
            0 >= prevExecutionContext ? prevExecutionContext = 0 : (prevDispatcher = suspenseConfig.busyDelayMs | 0, prevInteractions = now() - (10 * (1073741821 - prevInteractions) - (suspenseConfig.timeoutMs | 0 || 5e3)), prevExecutionContext = prevInteractions <= prevDispatcher ? 0 : prevDispatcher + prevExecutionContext - prevInteractions);
            if (10 < prevExecutionContext) {
              markRootSuspendedAtTime(root, expirationTime);
              root.timeoutHandle = scheduleTimeout(commitRoot.bind(null, root), prevExecutionContext);
              break;
...
function performSyncWorkOnRoot(root) {
  ...
  if ((executionContext & (RenderContext | CommitContext)) !== NoContext) throw Error("Should not already be working.");
  flushPassiveEffects();
  if (root !== workInProgressRoot || lastExpiredTime !== renderExpirationTime) prepareFreshStack(root, lastExpiredTime), startWorkOnPendingInteractions(root, lastExpiredTime);
  if (null !== workInProgress) {
    var prevExecutionContext = executionContext;
    executionContext |= RenderContext;
    var prevDispatcher = pushDispatcher(root),
...
function handleError(root$jscomp$0, thrownValue) {
  do {
    try {
      resetContextDependencies();
      resetHooks();
      if (null === workInProgress || null === workInProgress.return) return workInProgressRootExitStatus = RootFatalErrored, workInProgressRootFatalError = thrownValue, null;
      ...
      a: {
        ...
        if (null !== value && "object" === typeof value && "function" === typeof value.then) {
          ...
          if (0 === (sourceFiber.mode & 2)) {
            ...
            currentSource ? (sourceFiber.memoizedState = currentSource.memoizedState, sourceFiber.expirationTime = currentSource.expirationTime) : sourceFiber.memoizedState = null;
          }
...
function workLoopConcurrent() {
  for (; null !== workInProgress && !Scheduler_shouldYield();) workInProgress = performUnitOfWork(workInProgress);
}
function performUnitOfWork(unitOfWork) {
  var current$$1 = unitOfWork.alternate;
  0 !== (unitOfWork.mode & 8) ? (profilerStartTime = now$1(), 0 > unitOfWork.actualStartTime && (unitOfWork.actualStartTime = now$1()), current$$1 = beginWork$$1(current$$1, unitOfWork, renderExpirationTime), stopProfilerTimerIfRunningAndRecordDelta(unitOfWork, !0)) : current$$1 = beginWork$$1(current$$1, unitOfWork, renderExpirationTime);
...
function completeUnitOfWork(unitOfWork) {
  ...
  do {
    ...
    if (0 === (workInProgress.effectTag & 2048)) {
      if (0 === (workInProgress.mode & 8)) current$$1 = completeWork(current$$1, workInProgress, renderExpirationTime);else {
        var fiber = workInProgress;
        profilerStartTime = now$1();
        0 > fiber.actualStartTime && (fiber.actualStartTime = now$1());
        current$$1 = completeWork(current$$1, workInProgress, renderExpirationTime);
      ...
      if (1 === renderExpirationTime || 1 !== fiber.childExpirationTime) {
        ...
        if (0 !== (fiber.mode & 8)) {
          for (var actualDuration = fiber.actualDuration, treeBaseDuration = fiber.selfBaseDuration, shouldBubbleActualDurations = null === fiber.alternate || fiber.child !== fiber.alternate.child, child = fiber.child; null !== child;) {
      ...
      if (0 !== (workInProgress.mode & 8)) {
        ...
        for (newChildExpirationTime = workInProgress.child; null !== newChildExpirationTime;) fiber += newChildExpirationTime.actualDuration, newChildExpirationTime = newChildExpirationTime.sibling;
        workInProgress.actualDuration = fiber;
      }
      if (null !== current$$1) return current$$1.effectTag &= 2047, current$$1;
...
function commitRootImpl(root$jscomp$0, renderPriorityLevel$jscomp$0) {
  ...
  root$jscomp$0 === workInProgressRoot && (workInProgress = workInProgressRoot = null, renderExpirationTime = 0);
  ...
  if (null !== remainingExpirationTimeBeforeCommit) {
      ...
      for (var root = root$jscomp$0, renderPriorityLevel = renderPriorityLevel$jscomp$0; null !== nextEffect;) {
        ...
        switch (effectTag & 1038) {
          ...
          case 8:
            ...
            unmountHostComponents(root, current$$1$jscomp$0, renderPriorityLevel);
    ...
    nextEffect = remainingExpirationTimeBeforeCommit;
    do try {
      for (effectTag = root$jscomp$0, current$$1 = expirationTime; null !== nextEffect;) {
        var effectTag$jscomp$0 = nextEffect.effectTag;
        effectTag$jscomp$0 & 36 && commitLifeCycles(effectTag, nextEffect.alternate, nextEffect, current$$1);
        if (effectTag$jscomp$0 & 128) {
          ...
          if (null !== ref) {
            ...
            "function" === typeof ref ? ref(currentRef) : ref.current = currentRef;
          }
        }
        nextEffect = nextEffect.nextEffect;
      }
  ...
  if (effectTag$jscomp$0 = rootDoesHavePassiveEffects) rootDoesHavePassiveEffects = !1, rootWithPendingPassiveEffects = root$jscomp$0, pendingPassiveEffectsExpirationTime = expirationTime, pendingPassiveEffectsRenderPriority = renderPriorityLevel$jscomp$0;else for (nextEffect = remainingExpirationTimeBeforeCommit; null !== nextEffect;) renderPriorityLevel$jscomp$0 = nextEffect.nextEffect, nextEffect.nextEffect = null, nextEffect = renderPriorityLevel$jscomp$0;
  renderPriorityLevel$jscomp$0 = root$jscomp$0.firstPendingTime;
  if (0 !== renderPriorityLevel$jscomp$0) {
    if (null !== spawnedWorkDuringRender) for (remainingExpirationTimeBeforeCommit = spawnedWorkDuringRender, spawnedWorkDuringRender = null, ref = 0; ref < remainingExpirationTimeBeforeCommit.length; ref++) scheduleInteractions(root$jscomp$0, remainingExpirationTimeBeforeCommit[ref], root$jscomp$0.memoizedInteractions);
    schedulePendingInteractions(root$jscomp$0, renderPriorityLevel$jscomp$0);
  } else legacyErrorBoundariesThatAlreadyFailed = null;
  effectTag$jscomp$0 || finishPendingInteractions(root$jscomp$0, expirationTime);
...
function flushPassiveEffectsImpl() {
  ...
  var prevExecutionContext = executionContext;
  executionContext |= CommitContext;
  for (var prevInteractions = pushInteractions(root), effect = root.current.firstEffect; null !== effect;) {
    try {
      var finishedWork = effect;
      if (0 !== (finishedWork.effectTag & 512)) switch (finishedWork.tag) {
        case 0:case 11:case 15:case 22:
          commitHookEffectList(128, 0, finishedWork), commitHookEffectList(0, 64, finishedWork);
  ...
  switch (workInProgress.tag) {
    ...
    case 16:
      a: {
        ...
        throw Error("Element type is invalid. Received a promise that resolves to: " + renderState + ". Lazy element type must resolve to a class or function.");
      }
    ...
    case 3:
      ...
      if (null === current$$1 || null === updateExpirationTime) throw Error("If the root does not have an updateQueue, we should have already bailed out. This error is likely caused by a bug in React. Please file an issue.");
    ...
    case 10:
      a: {
        ...
        if (null !== getDerivedStateFromProps) {
          ...
          hasContext = is$1(oldValue, hasContext) ? 0 : ("function" === typeof updateExpirationTime._calculateChangedBits ? updateExpirationTime._calculateChangedBits(oldValue, hasContext) : 1073741823) | 0;
          if (0 === hasContext) {
...
function scheduleInteractions(root, expirationTime, interactions) {
  if (0 < interactions.size) {
    ...
    if (null !== pendingInteractionMap) pendingInteractionMap.onWorkScheduled(interactions, 1e3 * expirationTime + root.interactionThreadID);
  }
}
...
function finishPendingInteractions(root, committedExpirationTime) {
  ...
  try {
    var subscriber = tracing.__subscriberRef.current;
    if (null !== subscriber && 0 < root.memoizedInteractions.size) subscriber.onWorkStopped(root.memoizedInteractions, 1e3 * committedExpirationTime + root.interactionThreadID);
  } catch (error) {
    scheduleCallback(99, function () {
      throw error;
    });
...
function createWorkInProgress(current, pendingProps) {
  ...
  null === workInProgress ? (workInProgress = createFiber(current.tag, pendingProps, current.key, current.mode), workInProgress.elementType = current.elementType, workInProgress.type = current.type, workInProgress.stateNode = current.stateNode, workInProgress.alternate = current, current.alternate = workInProgress) : (workInProgress.pendingProps = pendingProps, workInProgress.effectTag = 0, workInProgress.nextEffect = null, workInProgress.firstEffect = null, workInProgress.lastEffect = null, workInProgress.actualDuration = 0, workInProgress.actualStartTime = -1);
...
var roots = new Map(),
    ...
    return function (_React$Component) {
      ...
      _proto.measureLayout = function (relativeToNativeNode, onSuccess, onFail) {
        ...
        if (null != maybeInstance && !maybeInstance.canonical) {
          if ("number" === typeof relativeToNativeNode) var relativeNode = relativeToNativeNode;else relativeToNativeNode._nativeTag && (relativeNode = relativeToNativeNode._nativeTag);
          null != relativeNode && ReactNativePrivateInterface.UIManager.measureLayout(findNodeHandle(this), relativeNode, mountSafeCallback_NOT_REALLY_SAFE(this, onFail), mountSafeCallback_NOT_REALLY_SAFE(this, onSuccess));
        }
      };
      _proto.setNativeProps = function (nativeProps) {
        try {
  ...
  dispatchCommand: function (handle, command, args) {
    null != handle._nativeTag && (handle._internalInstanceHandle ? nativeFabricUIManager.dispatchCommand(handle._internalInstanceHandle.stateNode.node, command, args) : ReactNativePrivateInterface.UIManager.dispatchViewManagerCommand(handle._nativeTag, command, args));
  },
