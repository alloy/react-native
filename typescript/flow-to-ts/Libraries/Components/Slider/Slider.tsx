yarn run v1.21.1
$ /Users/eloy/Code/ReactNative/react-native/node_modules/.bin/flow-to-ts Libraries/Components/Slider/Slider.js
'use strict';
 ...
 *
 */
const Slider = (props: Props, forwardedRef?: React.Ref<typeof SliderNativeComponent> | null | undefined) => {
  ...
  const onValueChangeEvent = onValueChange ? (event: Event) => {
    let userEvent = true;
  ...
  const onSlidingCompleteEvent = onSlidingComplete ? (event: Event) => {
    onSlidingComplete(event.nativeEvent.value);
  } : null;

  return <SliderNativeComponent {...localProps} // TODO: Reconcile these across the two platforms.
  enabled={!disabled} disabled={disabled} maximumValue={maximumValue} minimumValue={minimumValue} onChange={onChangeEvent} onResponderTerminationRequest={() => false} onSlidingComplete={onSlidingCompleteEvent} onStartShouldSetResponder={() => true} onValueChange={onValueChangeEvent} ref={forwardedRef} step={step} style={style} value={value} />;
};

const SliderWithRef: React.AbstractComponent<Props, React.ElementRef<typeof SliderNativeComponent>> = React.forwardRef(Slider);
